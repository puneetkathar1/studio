...
import { runKalshiIngestion } from "@/lib/server/ingestion";

export async function POST(req: NextRequest) {
  const route = "/api/ingest/kalshi";
  ...
  try {
    ...
    const result = await runKalshiIngestion(limit);
    console.log(JSON.stringify({ scope: "ingest.api", route, step: "success", ts: new Date().toISOString(), result }));
    return NextResponse.json({ ok: true, venue: "kalshi", ...result }, { status: 200 });
  } catch (error: any) {
    console.error(
      JSON.stringify({
        scope: "ingest.api",
